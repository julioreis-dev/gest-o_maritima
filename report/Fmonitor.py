...
class PlanMonitor:
    ...
    def formatacao(self):
        ...
        df_new['Valor Petro $'] = df_new[listcolumns[0]] / self.dolar
        df_new['Valor Petro $'] = df_new[['EMBARCAÇÃO\nPARCELA USD', 'Valor Petro $']].sum(axis=1).round(2)
        df_new['Valor PBL $'] = df_new[listcolumns[1]] / self.dolar
        df_new['Valor PBL $'] = df_new[['EMBARCAÇÃO PARCELA USD', 'Valor PBL $']].sum(axis=1).round(2)
        df_new['Total $'] = df_new[['Valor Petrobras $', 'Valor PBL $']].sum(axis=1).round(2)
        df_new = df_new[['Embarcação', 'Valor Petro $', 'Valor PBL $', 'Total $']]
        df = df_new.copy()
